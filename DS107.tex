...
The visual component is formulated as an object detection task. Given an input rice-field image, a YOLO-family detector predicts pest bounding boxes, class labels, and confidence scores. The detector is used not only to localize pests in the image but also to provide the pest-class prior required by the advisory module. Among the evaluated variants, YOLO11s is selected for the deployed pipeline because it provides the strongest image-level recognition balance while maintaining competitive localization performance.
