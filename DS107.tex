...
\subsection{Advisory Benchmark Dataset}
\label{sec:advisory_benchmark}

To evaluate the advisory component, we constructed a small in-domain
benchmark consisting of 40 manually designed Vietnamese questions.
The benchmark follows a balanced class--intent design: the 10 pest
classes in the detection taxonomy are each paired with four advisory
intents, namely symptoms, diagnosis, treatment, and prevention. These
intents are mapped to canonical knowledge sections:
\texttt{damage\_symptoms}, \texttt{field\_diagnosis},
\texttt{management\_ipm}, and \texttt{warnings}. Each benchmark item
therefore contains a question, target pest class, intent label, and
expected knowledge section.

Reference answers were manually prepared for all 40 questions using
the collected agricultural sources and domain-oriented pest management
knowledge. They were written as concise advisory responses rather
than copied verbatim from retrieved chunks. The benchmark is intended
for controlled in-domain analysis of retrieval and generation behavior;
it is not a substitute for a large-scale farmer-facing user study.

\section{Methodology}
\label{sec:methodology}

\subsection{Visual Detection Module}

The visual component is formulated as an object detection task. Given an input rice-field image, a YOLO-family detector predicts pest bounding boxes, class labels, and confidence scores. The detector is used not only to localize pests in the image but also to provide the pest-class prior required by the advisory module. The final deployed detector is selected from the evaluated YOLO variants based on the recognition and localization results reported in Section~\ref{sec:results}.

\subsection{Detection Output Representation}
...
The advisory index is built from 278 chunks, including 134 pest-specific chunks and 144 Vietnamese local-context chunks. Pest profiles are split by structured sections such as \texttt{identity}, \texttt{damage\_symptoms}, \texttt{field\_diagnosis}, \texttt{management\_ipm}, \texttt{chemical\_control}, and \texttt{warnings}; long section texts are split with a target length of 750 tokens, a maximum length of 900 tokens, and 100-token overlap. Each chunk stores metadata including \texttt{class\_id}, section label, source organization, source URL, source relation, and language. Embeddings are generated with \texttt{gemini-embedding-001} and stored in a JSONL vector index. The default advisory profile selects 12 candidates by cosine similarity and returns the top 4 reranked chunks; the retrieval benchmark overrides this setting to retrieve 20 candidates and evaluate the top 5 ranks. Reranking combines embedding score, lexical overlap, source quality, section-intent matching, and a source-diversity penalty. The generation module uses \texttt{gemini-2.5-flash-lite} with temperature 0.2 and a maximum output length of 1100 tokens.
...
Using the benchmark described in Section~\ref{sec:advisory_benchmark}, we evaluated the advisory module from two perspectives. For retrieval, we report Class Hit@5, Section Hit@5, and MRR@5 over the top-5 ranked chunks. Class Hit@5 measures whether the correct pest class appears in the top-5 retrieved chunks, Section Hit@5 measures whether the expected knowledge section is retrieved, and MRR@5 measures whether relevant chunks are ranked early. Because the advisory benchmark contains text questions rather than paired field images, the retrieval ablation simulates the YOLO prior by injecting the benchmark target class into the query/session context. This setting estimates the benefit of a correct detector-provided class prior; end-to-end evaluation with noisy detector outputs is left for future work.

For generation, system outputs were compared against the manually prepared reference answers using ROUGE-L~\citep{lin2004rouge}, BERTScore~\citep{zhang2020bertscore}, and semantic similarity. The No-RAG baseline uses the same language model with the question and target class identifier but without retrieved context, while the RAG setting uses the advisory pipeline to retrieve chunks for each question and supplies the retrieved context to the prompt. This makes the comparison conservative for RAG, since the No-RAG baseline is still given the target pest class. Semantic similarity was computed by encoding generated and reference answers with \texttt{paraphrase-multilingual-MiniLM-L12-v2} and measuring cosine similarity.
