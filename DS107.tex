...
The experiments were conducted on the bounding-box annotated rice pest dataset described in Section~\ref{sec:dataset}. The dataset was divided into training, validation, and test subsets. Data augmentation was applied exclusively to the training set, while the validation and test sets were kept unchanged to ensure a fair evaluation of model generalization.

To enhance model robustness, mitigate overfitting, and improve generalization, the following augmentation operations were applied: crop (0\% minimum zoom, 20\% maximum zoom); shear ($\pm10^\circ$ horizontal and vertical); saturation ($-25\%$ to $+25\%$); brightness ($-15\%$ to $+15\%$); exposure ($-10\%$ to $+10\%$); blur (up to 2.5 pixels); cutout (3 boxes, 10\% size each); and mosaic. This augmentation process expanded the training set from 2,210 to 6,630 images.
...
All experiments were conducted in the Kaggle Notebook environment using an NVIDIA Tesla T4 GPU. The maximum number of training epochs was set to 120, with early stopping (patience = 15). Detection-based variants used either $320 \times 320$ or $416 \times 416$ input resolution. For each experiment, the best checkpoint selected during validation was used for final evaluation on the test set.
